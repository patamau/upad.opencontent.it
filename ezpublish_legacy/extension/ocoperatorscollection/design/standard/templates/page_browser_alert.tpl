{def $browser = checkbrowser()
     $is_depracated = false()
     $deprecated = cond( ezini_hasvariable( 'Browser', 'BrowserDeprecato', 'ocoperatorscollection.ini' ), ezini( 'Browser', 'BrowserDeprecato', 'ocoperatorscollection.ini' ), false() )
...
{foreach $deprecated_browsers as $brower_type => $compare}
    {if eq( $browser['browser_working'], $brower_type )}
        {switch match=$compare[0]}
            ...
            {case match='lt'}
                {if $browser['browser_number']|le( $compare[1] )}
