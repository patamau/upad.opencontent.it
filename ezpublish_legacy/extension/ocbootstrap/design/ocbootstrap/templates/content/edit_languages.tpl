...
{if $show_existing_languages}
    ...
    {if $existing_languages_output|trim}
        <fieldset>
        <legend>{'Existing languages'|i18n('design/ocbootstrap/content/edit_languages')}</legend>
        <p>{'Select the language you want to use when editing the object.'|i18n('design/ocbootstrap/content/edit_languages')}:</p>
...
{if $nonexisting_languages_output|trim}
    <fieldset>
    <legend>{'New languages'|i18n('design/ocbootstrap/content/edit_languages')}</legend>
    <p>{'Select the language you want to add to the object.'|i18n('design/ocbootstrap/content/edit_languages')}:</p>

    {$nonexisting_languages_output}

    <p>{'Select the language the new translation will be based on.'|i18n('design/ocbootstrap/content/edit_languages')}:</p>

    <label>
        <input name="FromLanguage" type="radio" checked="checked" value="" /> {'Use an empty, untranslated draft'|i18n('design/ocbootstrap/content/edit_languages')}
    </label>
    <div class="labelbreak"></div>
...
{else}
    {if $show_existing_languages|not}
        {set $can_edit=false()}
        <p>{'You do not have permission to create a translation in another language.'|i18n('design/ocbootstrap/content/edit_languages')}</p>
...
<div class="block">
    {if $can_edit}
        <input class="button" type="submit" name="LanguageSelection" value="{'Edit'|i18n('design/ocbootstrap/content/edit_languages')}" />
    {/if}
    <input class="button" type="submit" name="CancelDraftButton" value="{'Cancel'|i18n('design/ocbootstrap/content/edit_languages')}" />
</div>
