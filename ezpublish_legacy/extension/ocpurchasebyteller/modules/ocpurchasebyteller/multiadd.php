...
$ObjectID = $http->variable( 'ContentObjectID', false );
$nodeID   = $http->variable( 'ContentNodeID', false );
$quantity = $http->variable( 'Quantity', false );
...
$OptionList = (array) $http->sessionVariable( "AddToBasket_OptionList_" . $ObjectID );

//echo $quantity .'-------'.$ObjectID.'-------'.$nodeID;
...
//eZDebug::writeNotice( $basket->attribute( 'items' ) );
...
// Recupero la lista del materiale didattico collegato e lo aggiungo al carrello
$includeClasses = array( 'materiale_didattico' );
$params = array(
    'ClassFilterType' => 'include',
    'ClassFilterArray' => $includeClasses
);

$materiale_didattico = eZContentObjectTreeNode::subTreeByNodeID( $params, $nodeID );


foreach ($materiale_didattico as $m) {
    $opRes = eZOperationHandler::execute( 'shop', 'addtobasket', array( 'basket_id' => $basket->attribute( 'id' ),
                                                                              'object_id' => $m->ContentObjectID,
                                                                              'option_list' => '',
                                                                              'quantity' => 1 ) );
}
