...
<div class="newsletter newsletter-configure">
    ...
    <form class="form-horizontal" role="form" name="configure" method="post" action={concat('/newsletter/configure/', $newsletter_user.hash)|ezurl}>
    ...
    {* for every newsletter_system check if there are newsletter_list where a subscription for current siteaccess is possible *}
        ...
        {else}
                ...
                <div class="form-group">
                  <label class="col-sm-2 control-label">{"Salutation"|i18n( 'cjw_newsletter/configure' )}:</label>
                  <div class="col-sm-10">
                    {foreach $available_saluation_array as $saluation_id => $salutataion_name}
                        <div class="radio"><input type="radio" name="Subscription_Salutation" value="{$saluation_id}"{if $newsletter_user.salutation|eq($saluation_id)} checked="checked"{/if} title="{$salutataion_name|wash}" />{$salutataion_name|wash}</div>
                    {/foreach}
                  </div>
                </div>
                ...
                {* First name. *}
                <div class="form-group">
                  <label class="col-sm-2 control-label" for="Subscription_FirstName">{"First name"|i18n( 'cjw_newsletter/configure' )}:</label>
                  <div class="col-sm-10">  
                    <input class="form-control" id="Subscription_FirstName" type="text" name="Subscription_FirstName" value="{$newsletter_user.first_name|wash}" title="{'First name of the subscriber.'|i18n( 'cjw_newsletter/configure' )}"{*cond( is_set( $user ), 'disabled="disabled"', '')*} />
                  </div>
                </div>

                {* Last name. *}
                <div class="form-group">
                  <label class="col-sm-2 control-label" for="Subscription_LastName">{"Last name"|i18n( 'cjw_newsletter/configure' )}:</label>
                  <div class="col-sm-10">  
                    <input class="form-control" id="Subscription_LastName" type="text" name="Subscription_LastName" value="{$newsletter_user.last_name|wash}"{*cond( is_set( $user ), 'disabled="disabled"', '')*} />
                  </div>
                </div>

                {* Email. *}
                <div class="form-group">
                  <label class="col-sm-2 control-label" for="Subscription_Email">{"Email"|i18n( 'cjw_newsletter/configure' )}*:</label>
                  <div class="col-sm-10">  
                    <input class="form-control" id="Subscription_Email" type="text" name="Subscription_Email" value="{$newsletter_user.email|wash}" title="{'Email of the subscriber.'|i18n( 'cjw_newsletter/configure' )}" {cond( is_set( $newsletter_user ), 'disabled="disabled"', '')} />
                  </div>
                </div>

                <div class="block">
                    <input class="button btn-primary pull-right" type="submit" name="ConfirmButton" value="{'Confirm'|i18n( 'cjw_newsletter/configure' )}" title="{'Add to subscription.'|i18n( 'cjw_newsletter/configure' )}" />
                </div>
