...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        <img src={$image.url|ezroot} {if $fluid}class="img-responsive{if $alignment|eq('center')} center-block{/if} {if $css_class}{$css_class|wash}{/if}"{/if} {if $hspace}hspace="{$hspace}"{/if} alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
        {if $href}</a>{/if}
    {/if}
