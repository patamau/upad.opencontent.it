...
<div class="attribute-header">
    <h1 class="long">{'Versions for <%object_name> [%version_count]'|i18n( 'design/ocbootstrap/content/history',, hash( '%object_name', $object.name, '%version_count', $list_count ) )|wash}</h1>
</div>

{if $list_count}
<table class="list" cellspacing="0">
<tr>
    <th class="tight"><img src={'toggle-button-16x16.gif'|ezimage} alt="{'Toggle selection'|i18n( 'design/ocbootstrap/content/history' )}" onclick="ezjs_toggleCheckboxes( document.versionsform, 'DeleteIDArray[]' ); return false;" /></th>
...
<tr class="{$seq}">
    ...
    <td>
        ...
        {else}
            <input type="checkbox" name="" value="" disabled="disabled" title="{'Version #%version_number cannot be removed because it is either the published version of the object or because you do not have permission to remove it.'|i18n( 'design/ocbootstrap/content/history',, hash( '%version_number', $version.version ) )}" />
        {/if}
    </td>
    ...
    {/foreach}
        ...
        {if and( $can_edit, $can_edit_lang )}
          {if eq( $version.status, 5 )}
            <input type="image" src={'copy-disabled.gif'|ezimage} name="" value="" disabled="disabled" title="{'There is no need to do a copies of untouched drafts.'|i18n( 'design/ocbootstrap/content/history' )}" />
          {else}
            <input type="hidden" name="CopyVersionLanguage[{$version.version}]" value="{$initial_language.locale}" />
            <input type="image" src={'copy.gif'|ezimage} name="HistoryCopyVersionButton[{$version.version}]" value="" title="{'Create a copy of version #%version_number.'|i18n( 'design/ocbootstrap/content/history',, hash( '%version_number', $version.version ) )}" />
          {/if}
        {else}
            <input type="image" src={'copy-disabled.gif'|ezimage} name="" value="" disabled="disabled" title="{'You cannot make copies of versions because you do not have permission to edit the object.'|i18n( 'design/ocbootstrap/content/history' )}" />
        {/if}
    {undef $can_edit_lang}
    </td>
    ...
    <td>
        {if and( array(0, 5)|contains($version.status), $version.creator_id|eq( $current_user.contentobject_id ), $can_edit ) }
            <input type="image" src={'edit.gif'|ezimage} name="HistoryEditButton[{$version.version}]" value="" title="{'Edit the contents of version #%version_number.'|i18n( 'design/ocbootstrap/content/history',, hash( '%version_number', $version.version ) )}" />
        {else}
            <input type="image" src={'edit-disabled.gif'|ezimage} name="HistoryEditButton[{$version.version}]" value="" disabled="disabled" title="{'You cannot edit the contents of version #%version_number either because it is not a draft or because you do not have permission to edit the object.'|i18n( 'design/ocbootstrap/content/history',, hash( '%version_number', $version.version ) )}" />
        {/if}
    </td>
...
<div class="left">
<input class="button" type="submit" name="RemoveButton" value="{'Remove selected'|i18n( 'design/ocbootstrap/content/history' )}" title="{'Remove the selected versions from the object.'|i18n( 'design/ocbootstrap/content/history' )}" />
...
<form action={concat( $module.functions.history.uri, '/', $object.id, '/' )|ezurl} method="post">
    ...
    <input class="button" type="submit" name="DiffButton" value="{'Show differences'|i18n( 'design/ocbootstrap/content/history' )}" />
</form>
...
{/if}
<input class="button" type="submit" name="BackButton" value="{'Back'|i18n( 'design/ocbootstrap/content/history' )}" />
</form>
</div>
</div>
...
<div class="attribute-header">
    <h1 class="long">{'Published version'|i18n( 'design/ocbootstrap/content/history' )}</h1>
</div>
...
<tr class="bglight">
    ...
    <td>
        ...
        {if and( $can_edit, $can_edit_lang )}
            <input type="image" src={'copy.gif'|ezimage} name="HistoryCopyVersionButton[{$published_item.version}]" value="" title="{'Create a copy of version #%version_number.'|i18n( 'design/ocbootstrap/content/history',, hash( '%version_number', $published_item.version ) )}" />
        {else}
            <input type="image" src={'copy-disabled.gif'|ezimage} name="" value="" disabled="disabled" title="{'You cannot make copies of versions because you do not have permission to edit the object.'|i18n( 'design/ocbootstrap/content/history' )}" />
        {/if}
        {undef $can_edit_lang}
...
<div class="attribute-header">
    <h1 class="long">{'New drafts [%newerDraftCount]'|i18n( 'design/ocbootstrap/content/history',, hash( '%newerDraftCount', $newerDraftVersionListCount ) )}</h1>
</div>
...
<tr class="{$seq}">
    ...
    {/foreach}
        ...
        {if and( $can_edit, $can_edit_lang )}
            ...
            <input type="image" src={'copy.gif'|ezimage} name="HistoryCopyVersionButton[{$draft_version.version}]" value="" title="{'Create a copy of version #%version_number.'|i18n( 'design/ocbootstrap/content/history',, hash( '%version_number', $draft_version.version ) )}" />
        {else}
            <input type="image" src={'copy-disabled.gif'|ezimage} name="" value="" disabled="disabled" title="{'You cannot make copies of versions because you do not have permission to edit the object.'|i18n( 'design/ocbootstrap/content/history' )}" />
        {/if}
    {undef $can_edit_lang}
    ...
    <td>
        {if and( array(0, 5)|contains($draft_version.status), $draft_version.creator_id|eq( $current_user.contentobject_id ), $can_edit ) }
            <input type="image" src={'edit.gif'|ezimage} name="HistoryEditButton[{$draft_version.version}]" value="" title="{'Edit the contents of version #%version_number.'|i18n( 'design/ocbootstrap/content/history',, hash( '%version_number', $draft_version.version ) )}" />
        {else}
            <input type="image" src={'edit-disabled.gif'|ezimage} name="HistoryEditButton[{$draft_version.version}]" disabled="disabled" value="" title="{'You cannot edit the contents of version #%version_number either because it is not a draft or because you do not have permission to edit the object.'|i18n( 'design/ocbootstrap/content/history',, hash( '%version_number', $draft_version.version ) )}" />
        {/if}
    </td>
...
{else}
<div class="block">
<p>{'This object does not have any drafts.'|i18n( 'design/ocbootstrap/content/history' )}</p>
</div>
{/if}
...
<div class="attribute-header">
    <h1 class="long">{'Differences between versions %oldVersion and %newVersion'|i18n( 'design/ocbootstrap/content/history',, hash( '%oldVersion', $oldVersion, '%newVersion', $newVersion ) )}</h1>
</div>
...
<input class="button" type="submit" value="{'Back to history'|i18n( 'design/ocbootstrap/content/history' )}" />
</form>
</div>
</div>
{/if}
