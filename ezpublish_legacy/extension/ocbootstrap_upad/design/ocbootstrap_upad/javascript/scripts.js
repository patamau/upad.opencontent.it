...
(function($){
	...
	(function(){
		...
		if(ncc.length){
			...
			$('.partner_carousel_prev').on('click',function(){
				nc.trigger('owl.prev');
			});
			$('.partner_carousel_next').on('click',function(){
				nc.trigger('owl.next');
			});
		}
	})();
	...
	(function(){
		...
		var nec = $('.ne_carousel');
		if(nec.length){
			var ne = nec.owlCarousel({
				itemsCustom : [[1199,2],[992,2],[768,1],[575,1],[300,1]],
				autoPlay : false,
				slideSpeed : 1000,
				autoHeight : true
			});

			$('.ne_prev').on('click',function(){
				ne.trigger('owl.prev');
			});

			$('.ne_next').on('click',function(){
				ne.trigger('owl.next');
			});
		}
	...
	(function(){
		if($('.news_carousel').length){
			var blog = $('.news_carousel').owlCarousel({
				items : 2,
				stopOnHover : true,
				slideSpeed : 600,
				autoHeight : true,
	...
	(function(){
		...
		if($('#timerange').length){
			timerange = $('#timerange').slider({
				...
				max: 24,
				slide : function(event ,ui){
					$(this).next().find('.first_limit').val(ui.values[0] +':00');
					$(this).next().find('.last_limit').val(ui.values[1] +':00');
				}
			});
		}
